...
def get_symmetric_states(obs=None, actions=None, env=None, obs_type='policy'):
    ...
    num_stack = 10
    frame_feat_actor  = 47  # per-frame actor features (policy)
    frame_feat_critic = 52  # per-frame critic features: 78 (actor-like) + 3 (root vel) + 2 (contact)

    # L/R permutation for 12-DoF blocks 摆动关节
    swap_joint_idx = [1, 0, 3, 2, 5, 4, 7, 6, 9, 8, 11, 10]
    ...
    for idx in [2, 3, 4, 5, 10, 11]:
        flip_sign_1f[11 + idx] = -1.0
    for idx in [2, 3, 4, 5, 10, 11]:
        flip_sign_1f[23 + idx] = -1.0
    ...
    joint_vel_perm = [idx + 23 for idx in swap_joint_idx] # 关节速度左右对调

    for i in range(12):
        flip_idx_1f[11 + i] = joint_pos_perm[i]
        flip_idx_1f[23 + i] = joint_vel_perm[i]
    # action block in obs
    action_perm = swap_joint_idx
    for i in range(12):
        flip_idx_1f[35 + i] = 35 + action_perm[i]


    # === 动作符号反转 ===
    # 这些关节的动作在镜像时需要取反（waist、roll/yaw）
    neg_action_idx = [2, 3, 4, 5, 10, 11]
    for idx in neg_action_idx:
        flip_sign_1f[35 + idx] = -1.0
    ...
    if obs is None and actions is not None and obs_type == 'policy':
        ...
        # 为动作镜像也添加符号反转
        action_sign = torch.ones(12, device=device)
        action_sign[torch.tensor( [2, 3, 4, 5, 10,11], device=device)] = -1.0
